...
def init_images():
    ...
    images = {
        "towers": tower_images,
        "enemies": {
            "normal": load_image("sprites/enemies/normal.png"),
            "fast": load_image("sprites/enemies/fast.png"),
            "tank": load_image("sprites/enemies/tank.png"),
            "swarm": load_image("sprites/enemies/swarm.png"),
            "shield": load_image("sprites/enemies/shield.png"),
            "armored": load_image("sprites/enemies/armored.png"),
            "flying": load_image("sprites/enemies/flying.png"),
            "boss": load_image("sprites/enemies/boss.png"),
        },
        "terrain": {
...
class Enemy:
    def __init__(self, hp, speed, reward, kind="normal", shield_hits=0, flying=False, boss=False, death_spawns=0, affix=None, path_index=0):
        ...
        self.radius = 26 if boss else 15
    ...
    def draw(self):
        ...
        sprite_key = "boss" if self.boss else "flying" if self.flying else self.kind
        sprite = images.get("enemies", {}).get(sprite_key) or images.get("enemies", {}).get("normal")
...
class Tower:
    ...
    def draw(self):
        ...
        sprite = None
        if self.tower_type:
            frame = "fire" if self.fire_anim_timer > 0 else "idle"
            sprite = images.get("towers", {}).get(self.tower_type, {}).get(frame)
...
class Projectile:
    ...
    def hit_target(self):
        ...
        dealt = self.target.take_damage(self.damage, damage_type)
        if shield_before > 0:
        ...
        if self.tower_type == "sniper":
            ...
            for enemy in enemies:
                if enemy != self.target and dist((self.target.x, self.target.y), (enemy.x, enemy.y)) <= splash_radius:
                    dealt = enemy.take_damage(splash_damage, "explosive")
...
def draw_terrain():
    ...
    grass = images.get("terrain", {}).get("grass")
    grass_dark = images.get("terrain", {}).get("grass_dark")
    if grass:
...
def draw_path():
    road = images.get("terrain", {}).get("road")
    road_color = road.get_at((8, 8))[:3] if road else (126, 98, 62)
    road_edge = images.get("terrain", {}).get("road_edge")
...
def get_wave_affix(wave_number=None):
    preview_wave = wave if wave_number is None else wave_number
    if preview_wave < 8 or preview_wave % 5 == 0:
        return None
    affixes = [None, "haste", "regen", "armored", "split"]
    return affixes[preview_wave % len(affixes)]
